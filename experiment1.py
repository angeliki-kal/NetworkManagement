...
####################
phase1_car0 = 'phase1_car0.data'
phase1_car3 = 'phase1_car3.data'
phase1_car3_s = 'phase1_car3_s.data'
phase1_car3_client_ping = 'phase1_car3_client_ping.data'
phase1_car0_car3_ping = 'phase1_car0_car3_ping.data'
phase1_client_out = 'phase1_client.data'
####################
phase2_car0_out = 'phase2_car0.data'
phase2_client_out = 'phase2_client.data'
phase2_ping = 'phase2_ping.data'
####################
phase3_c0 = 'phase3_car0.data'
phase3_client = 'phase3_client.data'
...
def graphic():
    ...
    if not os.path.exists('output'):
        os.makedirs('output')
    print"Phase 1:"
    tjp = pd.read_csv('c0-c3.p1', delim_whitespace=True, header=None)
    tjp2 = pd.read_csv('c3-client.p1', delim_whitespace=True, header=None)
#latency
    l = pd.read_csv('c0-c3-ping.p1', delim_whitespace=True, header=None)
    l2 = pd.read_csv('c3-client-ping.p1', delim_whitespace=True, header=None)
    ...
    plt.xlabel('time (sec)')
    plt.plot(latency_p1)
    latency_fig.savefig('output/latency_phase1.png')
 #packet loss
    packet_loss_p1 = []
    ...
    packet_loss_fig.savefig('output/packet_loss_phase1.png')
 #throughput
    ...
    throughput_fig.savefig('output/throughput_phase1.png')
 #jitter     
    ...
    plt.xlabel('Seconds')
    plt.plot(jitter_p1)
    jitter_fig.savefig('output/jitter_phase1.png')

    print "Phase 2:"
    #latency
    l = pd.read_csv('c0-client-ping.p2', delim_whitespace=True, header=None)
    ...
    plt.xlabel('time (sec)')
    plt.plot(latency_p2)
    latency_fig.savefig('output/latency_phase2.png')
 #Throughput   
    tjp = pd.read_csv('c0-client.p2', delim_whitespace=True, header=None)
    ...
    throughput_fig.savefig('output/throughput_phase2.png')
#Packet Loss
    packet_loss_p2 = []
    ...
    packet_loss_fig.savefig('output/packet_loss_phase2.png')
 #Jitter
    ...
    plt.xlabel('time (sec)')
    plt.plot(jitter_p2)
    jitter_fig.savefig('output/jitter_phase2.png')
 
    print "Phase 3:"
    #Throughput
    tjp = pd.read_csv('c0-client.p3', delim_whitespace=True, header=None)
    throughput_p3 = tjp[0].tolist()
    throughput_fig = plt.figure()
    throughput_fig.suptitle('[Phase 3]: Throughput', fontsize=20)
    plt.ylabel('kbps')
    plt.xlabel('time (sec)')
    plt.plot(throughput_p3)
    throughput_fig.savefig('output/throughput_phase3.png')
 #Latency
    l = pd.read_csv('c0-client-ping.p2', delim_whitespace=True, header=None)
    latency_p3 = l[0].tolist()
    latency_fig = plt.figure()
    latency_fig.suptitle('[Phase 3]: latency', fontsize=20)
    plt.ylabel('ms')
    plt.xlabel('Seconds')
    plt.plot(latency_p3)
    latency_fig.savefig('output/latency_phase3.png')
   #Packet Loss
    packet_loss_p3 = []
    for packets in tjp[2].tolist():
        packet_loss_p3.append(int(packets.rpartition('/')[0]))
    packet_loss_fig = plt.figure()
    packet_loss_fig.suptitle('[Phase 3]: Packet Loss', fontsize=20)
    plt.ylabel('Packets')
    plt.xlabel('Seconds')
    plt.plot(packet_loss_p3)
    packet_loss_fig.savefig('output/packet_loss_phase3.png')
    #Jitter
    jitter_p3 = tjp[1].tolist()
    jitter_fig = plt.figure()
    jitter_fig.suptitle('[Phase 3] Jitter', fontsize=20)
    plt.ylabel('jitter (ms)')
    plt.xlabel('time (sec)')
    plt.plot(jitter_p3)
    jitter_fig.savefig('output/jitter_phase3.png')
...
def apply_experiment(car,client,switch):
    ...
    car[3].cmd('iperf -s -u -i 1 >> %s &' % phase1_car3_s)
    time.sleep(1)
    car[0].cmd('ping 192.168.1.7 -c %d >> %s &' % (x,phase1_car0_car3_ping))
    car[0].cmd('iperf -c 192.168.1.7 -u -i 1 -t %d >> %s &' % (x,phase1_car0))
    car[3].cmd('ping 200.0.10.2 -c %d >> %s &' % (x,phase1_car3_client_ping))
    car[3].cmd('iperf -c 200.0.10.2 -u -i 1 -t %d >> %s' % (x,phase1_car3))
    time.sleep(1)
    car[0].cmd('grep -v "datagrams received out-of-order" phase1_car3_s.data | head -16 | tail -9 | tr -s " " | cut -d " " -f6,10,12-13 > c0-c3.p1 ')
    car[0].cmd('grep -v "datagrams received out-of-order" phase1_car3_s.data | head -17 | tail -1 | tr -s " " | cut -d " " -f5,9,11-12 >> c0-c3.p1 ')
    car[0].cmd('grep -v "datagrams received out-of-order" phase1_client.data | head -16 | tail -9 | tr -s " " | cut -d " " -f6,10,12-13 > c3-client.p1 ')
    car[0].cmd('grep -v "datagrams received out-of-order" phase1_client.data | head -17 | tail -1 | tr -s " " | cut -d " " -f5,9,11-12 >> c3-client.p1 ')
    car[0].cmd('tail -14 phase1_car0_car3_ping.data | head -10 | cut -d "=" -f4 > c0-c3-ping.p1')
    car[0].cmd('tail -14 phase1_car3_client_ping.data | head -10 | cut -d "=" -f4 > c3-client-ping.p1')
    ...
    car[0].cmd('iperf -c 200.0.10.2 -u -i 1 -t %d >> %s' % (x,phase2_car0_out))
    time.sleep(2)
    car[0].cmd('grep -v "datagrams received out-of-order" phase2_client.data | head -16 | tail -9 | tr -s " " | cut -d " " -f6,10,12-13 > c0-client.p2 ')
    car[0].cmd('grep -v "datagrams received out-of-order" phase2_client.data | head -17 | tail -1 | tr -s " " | cut -d " " -f5,9,11-12 >> c0-client.p2 ')
    ...
    client.cmd('iperf -s -u -i 1 >> %s &' % phase3_client)
    time.sleep(1)
    car[0].cmd('ping 200.0.10.2 -c %d >> %s &' % (x,phase3_ping))
    car[0].cmd('iperf -c 200.0.10.2 -u -i 1 -t %d >> %s' % (x,phase3_c0))
    time.sleep(2)
    car[0].cmd('grep -v "datagrams received out-of-order" phase3_client.data | head -16 | tail -9 | tr -s " " | cut -d " " -f6,10,12-13 > c0-client.p3')
    car[0].cmd('grep -v "datagrams received out-of-order" phase3_client.data | head -17 | tail -1 | tr -s " " | cut -d " " -f5,9,11-12 >> c0-client.p3')
...
def topology():
    ...
    car[0].cmd("vlc -vvv bunnyMob.mp4  'duplicate{dst=rtp{dst=200.0.10.2,port=5004,mux=ts},dst=display}' :sout-keep &")
    client.cmd("vlc rtp://200.0.10.2:5004 &")
